...
export class Node {
  ...
  getWeight(): number {
    var weight =
      typeof this.weight === "function" ? this.weight(this) : this.weight;
...
export class Topology extends React.Component<Props, {}> {
  ...
  private createSVG() {
    ...
    // levels group
    this.gLevels = this.g.append("g").attr("class", "levels");

    // groups group, yes read it correctly groups group
    this.gGroups = this.g.append("g").attr("class", "groups");

    // hiera links group
    this.gHieraLinks = this.g.append("g").attr("class", "hiera-links");

    // link overlay group, like highlight
    this.gLinkOverlays = this.g.append("g").attr("class", "link-overlays");

    // non-hiera links group
    this.gLinks = this.g.append("g").attr("class", "links");

    // link labels
    this.gLinkLabels = this.g.append("g").attr("class", "link-labels");

    // link wrapper group, used to catch mouse event
    this.gLinkWraps = this.g.append("g").attr("class", "link-wraps");

    // groups group, yes read it correctly groups group
    this.gGroupButtons = this.g.append("g").attr("class", "group-buttons");

    // nodes group
    this.gNodes = this.g.append("g").attr("class", "nodes");

    // levels group
    this.gLevelLabels = this.g.append("g").attr("class", "level-labels");

    // context menu group
    this.gContextMenu = this.svg.append("g").attr("class", "context-menu");
  ...
  addLink(
    ...
    this.links.set(
      id,
      new Link(id, tags, node1, node2, data, { selected: false })
    );
    ...
    tags.forEach((tag) => {
      ...
      if (!this.linkTagStates.has(tag)) {
        let mode = this.props.defaultLinkTagMode
          ? this.props.defaultLinkTagMode(tag)
          : LinkTagState.EventBased;
  ...
  private groupify(node: NodeWrapper): Map<string, NodeWrapper> {
    ...
    var nodeTypeGID = (
      ...
      var nodeType = this.props.groupType
        ? this.props.groupType(child)
        : child.data.Type;
      if (!nodeType) {
        return;
      }
    ...
    node.children.forEach((child) => {
      ...
      if (!wrapper) {
        var state = this.groupStates.get(gid) || {
          expanded: false,
          selected: false,
          mouseover: false,
          groupOffset: 0,
        ...
        var name = this.props.groupName
          ? this.props.groupName(child.wrapped)
          : nodeType + "(s)";
        ...
        var wrapped = new Node(
          ...
          state,
          () => {
            return child.wrapped.getWeight();
          }
        );
          ...
          if (wrapper.wrapped.state.groupFullSize) {
            ...
            children = children.concat(
              wrapper.children.splice(
                wrapper.wrapped.state.groupOffset,
                groupSize
              )
            );
          }
    ...
    if (node.state.expanded) {
      ...
      if (this.props.sortNodesFnc) {
        cloned.children.sort((a, b) =>
          this.props.sortNodesFnc(a.wrapped, b.wrapped)
        );
      }
  ...
  private normalizeTree(node: Node): NodeWrapper | null {
    ...
    let normalizeTreeHeight = (
      root: NodeWrapper,
      node: NodeWrapper,
      weight: number,
      currDepth: number,
      cache: { chains: Map<string, { first: NodeWrapper; last: NodeWrapper }> }
    ) => {
      ...
      if (
        nodeWeight === weight &&
        node.parent &&
        node.parent.wrapped.getWeight() !== weight
      ) {
  ...
  private visibleLinks(): Array<Link> {
    ...
    this.links.forEach((link: Link) => {
      ...
      if (
        ...
        if (
          link.tags.some(
            (tag) => this.linkTagStates.get(tag) !== LinkTagState.Hidden
          )
        ) {
          links.push(
            new Link(link.id, link.tags, source, target, link.data, link.state)
          );
        }
      }
    });
  ...
  private levelNodes(): Array<LevelNodes> {
    ...
    Array.from(this.d3nodes.values()).forEach((node) => {
      if (
        node.data.wrapped !== this.root &&
        node.data.type !== WrapperType.Hidden
      ) {
  ...
  private unselectAllLinks() {
    ...
    this.gLinkOverlays.selectAll(".link-overlay-selected").each(function () {
      ...
      select("#link-overlay-" + id).style(
        "opacity",
        self.isLinkVisible(l) ? 1 : 0
      );
  ...
  private viewSize(): { width: number; height: number } {
    ...
    return {
      width: parent.clientWidth || parent.parentNode.clientWidth,
      height: parent.clientHeight || parent.parentNode.clientHeight,
    };
  }
  ...
  zoomFit() {
    ...
    var scale =
      0.65 / Math.max(width / viewSize.width, height / viewSize.height);
    if (scale > 1) {
      scale = 1;
    }
    ...
    var t = zoomIdentity
      ...
      .scale(scale);
    this.svg.transition().duration(animDuration).call(this.zoom.transform, t);
  }
  ...
  private showNodeContextMenu(d: D3Node) {
    ...
    if (this.props.onShowNodeContextMenu) {
      ...
      var x = event.x - divBB.left,
        y = event.y - divBB.top;

      var g = this.gContextMenu.append("g").style("opacity", 0);
      g.transition().duration(300).style("opacity", 1);
      var rect = g.append("rect").attr("filter", "url(#drop-shadow)");
      ...
      var dy = 0,
        rects = new Array<Selection<SVGGElement, {}, null, undefined>>();
  ...
  private neighborLinks(node: NodeWrapper, links: Array<Link>): Array<Link> {
    ...
    for (let link of links) {
      if (
        link.source.id === node.wrapped.id ||
        link.target.id === node.wrapped.id
      ) {
        neighbors.push(link);
      }
    }
  ...
  private showNode(node: Node) {
    ...
    while (id) {
      ...
      if (d) {
        ...
        if (group) {
          let offset = group.wrapped.children.findIndex(
            (child) => child.id === id
          );
          if (offset >= 0) {
  ...
  private moveTo(x: number, y: number) {
    ...
    var t = zoomIdentity
      .translate(
        viewSize.width / 2 - scale * x,
        viewSize.height / 2 - scale * y
      )
      .scale(scale);
  ...
  pinNode(node: Node, active) {
    ...
    select("#node-pinned-" + node.id).style("opacity", active ? 1 : 0);
  ...
  private highlightNeighborLinks(d: D3Node, active: boolean) {
    ...
    for (let link of links) {
      if (active || !this.isLinkNodeSelected(link)) {
        select("#link-" + link.id).style("opacity", (d: Link) =>
          this.isLinkVisible(d) ? 1 : opacity
        );
        select("#link-label-" + link.id).style("opacity", (d: Link) =>
          this.isLinkVisible(d) ? 1 : opacity
        );
        select("#link-overlay-" + link.id).style(
          "opacity",
          link.state.selected || opacity
        );
      }
    }
  }
  ...
  private isLinkVisible(link: Link): boolean {
    ...
    return link.tags.some(
      (tag) =>
        this.linkTagStates.get(tag) === LinkTagState.Visible ||
        (this.linkTagStates.get(tag) === LinkTagState.EventBased &&
          (link.source.state.selected ||
            link.target.state.selected ||
            link.source.state.mouseover ||
            link.target.state.mouseover))
    );
  }
  ...
  private showLevelLabel(d: LevelRect) {
    ...
    var text = label.select("text").attr("style", "");
  ...
  private renderLevels() {
    ...
    var levelLabel = this.gLevelLabels
      .selectAll("g.level-label")
      .data(this.levelRects, (d: LevelRect) => "level-label-" + d.weight);
    var levelLabelEnter = levelLabel
      .enter()
      .append("g")
      ...
      .attr(
        "transform",
        (d: LevelRect) => `translate(${-self.absTransformX},${d.bb.y})`
      );
    levelLabelEnter
      .append("rect")
      .attr("width", 40)
    ...
    levelLabelEnter
      ...
      .attr("dx", 18)
      .text(
        (d: LevelRect) => self.weightTitles.get(d.weight) || "Level " + d.weight
      );
    levelLabel.exit().remove();

    var level = this.gLevels
      .selectAll("g.level")
      .data(this.levelRects, (d: LevelRect) => "level-" + d.weight)
      .interrupt();
    var levelEnter = level
      .enter()
      .append("g")
  ...
  private renderHieraLinks(root: any) {
    ...
    var hieraLink = this.gHieraLinks
      .selectAll("path.hiera-link")
      .data(root.links(), (d: any) => d.source.data.id + d.target.data.id)
      .interrupt();
    var hieraLinkEnter = hieraLink
      .enter()
    ...
    hieraLinkEnter.transition().duration(animDuration).style("opacity", 1);
  ...
  private renderGroups() {
    ...
    const handleCurlyBraces = (g: any, d: NodeWrapper, animated: boolean) => {
      ...
      if (animated) {
        curly = curly.transition().duration(animDuration);
      }
      ...
      if (d3node) {
        let xEnd = d.wrapped.state.expanded
          ? d3node.x + self.nodeWidth / 2
          : x2;
        let junction = d.wrapped.state.expanded
          ? " L " + xEnd + " " + y1 + " "
          : right;
        curly.attr(
          "d",
    ...
    groupEnter.transition().duration(animDuration).style("opacity", 1);

    groupEnter.append("path").attr("class", "curly-brace-bg");
    ...
    groupEnter.append("path").attr("class", "curly-brace curly-brace-left");

    groupEnter.append("path").attr("class", "curly-brace curly-brace-right");

    groupEnter.each(function (d) {
      handleCurlyBraces(select(this), d, false);
    });
    group.each(function (d) {
      handleCurlyBraces(select(this), d, true);
    });

    group.transition().duration(animDuration).style("opacity", 1);
    ...
    const handleIcon = (
      ...
      var text = gIcon.select("text").attr("x", 0).attr("y", 0);
      ...
      gIcon
        ...
        .attr("transform", (d: D3Node) =>
          d3node ? `translate(${d3node.x + this.nodeWidth / 2},${y + dy})` : ``
        );
    };

    var groupButton = this.gGroupButtons
      .selectAll("g.group-button")
      .interrupt()
      .data(Array.from(this.groups.values()), (d: NodeWrapper) => d.id);
    var groupButtonEnter = groupButton
      .enter()
      .append("g")
    ...
    var leftIcon = groupButtonEnter
      .append("g")
      .attr("class", "curly-icon curly-left-icon")
      .style("opacity", 0);
    leftIcon.append("rect").attr("rx", 5).attr("ry", 5);
    leftIcon
      .append("text")
      .text("\uf191")
    ...
    var rightIcon = groupButtonEnter
      .append("g")
      .attr("class", "curly-icon curly-right-icon")
      .style("opacity", 0);
    rightIcon.append("rect").attr("rx", 5).attr("ry", 5);
    rightIcon
      .append("text")
      .text("\uf152")
      .on("click", (d: NodeWrapper) => {
    ...
    var fullIcon = groupButtonEnter
      .append("g")
      .attr("class", "curly-icon curly-full-icon")
      .style("opacity", 0);
    fullIcon.append("rect").attr("rx", 5).attr("ry", 5);
    fullIcon
      .append("text")
      .text("\uf0fe")
    ...
    const handleIcons = (g: any, d: NodeWrapper, animated: boolean) => {
      ...
      handleIcon(
        g.select("g.curly-right-icon"),
        d,
        25,
        animated,
        d.wrapped.state.groupFullSize ||
          d.wrapped.state.groupOffset + size >= d.wrapped.children.length
      );
      handleIcon(g.select("g.curly-full-icon"), d, 75, animated, false);
    };

    groupButtonEnter.each(function (d: NodeWrapper) {
      handleIcons(select(this), d, true);
    });
    groupButton.each(function (d: NodeWrapper) {
      handleIcons(select(this), d, true);
    });
  ...
  private renderNodes(root: any) {
    ...
    var nodeEnter = node
      ...
      .filter(
        (d: D3Node) =>
          d.data.type !== WrapperType.Hidden && d.data.wrapped !== this.root
      )
      .append("g")
    ...
    nodeEnter.transition().duration(animDuration).style("opacity", 1);
    ...
    var wrapText = (
      text: Selection<SVGTextElement, any, null, undefined>,
      lineHeight: number,
      width: number
    ) => {
      text.each(function () {
    ...
    var exco = nodeEnter
      ...
      .style("opacity", (d: D3Node) =>
        d.data.wrapped.children.length > 0 ? 1 : 0
      );
    ...
    node
      .select("text.node-exco-children")
      .filter((d: D3Node) => d.data.wrapped.children.length > 0)
      .text((d: D3Node) => num(d.data));
  ...
  private renderLinks() {
    ...
    const linkOverlayClass = (d: Link) =>
      new Array<string>()
        .concat("link-overlay", d.state.selected ? "link-overlay-selected" : "")
        .join(" ");

    var linkOverlay = this.gLinkOverlays
      .selectAll("path.link-overlay")
      .interrupt()
      .data(visibleLinks, (d: Link) => d.id);
    var linkOverlayEnter = linkOverlay
      .enter()
      .append("path")
    ...
    linkOverlay
      ...
      .style("opacity", (d: Link) =>
        d.state.selected ||
        this.isLinkNodeSelected(d) ||
        this.isLinkNodeMouseOver(d)
          ? 1
          : 0
      )
      .attr("d", linker);

    var link = this.gLinks
      .selectAll("path.link")
      .interrupt()
      .data(visibleLinks, (d: Link) => d.id);
    ...
    const linkClass = (d: Link) => {
      ...
      return classes
        .concat("link", attrs.classes, attrs.directed ? directedClass(d) : "")
        .join(" ");
    };
    ...
    var linkLabel = this.gLinkLabels
      .selectAll("text.link-label")
      .interrupt()
      .data(
        visibleLinks.filter((d: Link) => this.props.linkAttrs(d).label),
        (d: Link) => d.id
      );
    ...
    var linkWrap = this.gLinkWraps
      .selectAll("path.link-wrap")
      .interrupt()
      .data(visibleLinks, (d: Link) => d.id);
    var linkWrapEnter = linkWrap
      .enter()
      .append("path")
      ...
      .on("mouseover", (d: Link) => {
        if (this.isLinkVisible(d)) {
          select("#link-overlay-" + d.id).style("opacity", 1);
        }
      })
      .on("mouseout", (d: Link) => {
        if (!d.source.state.selected && !d.target.state.selected) {
          select("#link-overlay-" + d.id).style("opacity", (d: Link) =>
            d.state.selected ? 1 : 0
          );
        }
      });
    ...
    linkWrap.transition().duration(animDuration).attr("d", linker);
  }
